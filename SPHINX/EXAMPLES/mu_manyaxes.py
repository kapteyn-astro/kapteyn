...
# 1. Read the header
f = maputils.FITSimage("manyaxes.fits")

# 2. Create a Matplotlib Figure and Axes instance
#fig = plt.figure(figsize=(7,7))
figsize=f.get_figsize(ysize=7)
fig = plt.figure(figsize=figsize)
frame = fig.add_subplot(1,1,1)

# 4. Create a graticule
f.set_imageaxes(1,4)
mplim = f.Annotatedimage(frame)
grat = mplim.Graticule(starty=1000, deltay=10)

# 5. Show the calculated world coordinates along y-axis
print "The world coordinates along the y-axis:", grat.ystarts

# 6. Show header information in attributes of the Projection object
print "CRVAL, CDELT from header:", grat.gmap.crval, grat.gmap.cdelt

# 7. Set a number of properties of the graticules and plot axes
